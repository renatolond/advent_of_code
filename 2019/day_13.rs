...
unsafe fn real_main() {
    ...
    loop {
        let (x, ptr, pos_ptr) = machine(None, instruction_pointer, total_instructions, pos_pointer);
        if ptr == None {
            println!("Machine out!");
            break;
        }
        let (y, ptr2, pos_ptr2) = machine(None, ptr, total_instructions, pos_ptr);
        let (t_id, ptr3, pos_ptr3) = machine(None, ptr2, total_instructions, pos_ptr2);
